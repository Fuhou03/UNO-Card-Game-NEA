...
class Button():
    ...
    def get_text(self, surface, ev):
        ''' Gets your inputted text and stores it into the text variable '''

        if ev.key == pygame.K_BACKSPACE:
            self.text = self.text[:-1]  # Till 2nd last character
        else:
            self.text += ev.unicode  # Unicode is the info of the button pressed.
            # Adds the letter you pressed onto screen - allows you to write onto screen
